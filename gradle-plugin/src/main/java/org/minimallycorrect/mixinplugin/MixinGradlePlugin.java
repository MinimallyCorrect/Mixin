...
public class MixinGradlePlugin implements Plugin<Project> {
	...
	public void apply(@NonNull Project project) {
		// TODO: Use artifact transforms when possible (gradle >= 5.5)
		// https://docs.gradle.org/current/userguide/artifact_transforms.html#sec:abm_artifact_transforms
		settings = new Settings();
		project.getExtensions().add("mixin", settings);
		val mixinsTask = project.task("applySubprojectMixins");
		project.getTasks().getByName("compileJava").dependsOn(mixinsTask);
